...
public class UI_Game : MonoBehaviour
{
	...
	public UnityEngine.GameObject tradeSystem;
    private Inventory craftSystemInventory;
    private Inventory mainInventory;
    private Inventory characterSystemInventory;
	private Inventory TradeSystemInventory;
    private UI_AvatarState avatarState;
	private UI_trade Trade;
	...
	void Start () {
		...
		KBEngine.Event.registerOut("OpenTradeWindows", this, "Open_TradeWindows");
        //inventory
		...
		if (canvas.transform.Find("Panel - trade") != null)
			tradeSystem = canvas.transform.Find("Panel - trade").gameObject;
		...
		if (tradeSystem != null)
			Trade = tradeSystem.GetComponent<UI_trade>();
	...
	public void OnTrade()
    {
		if (!tradeSystem.activeSelf)
        {
			...
			if (ui_target != null && ui_target.GE_target != null) {
				...
				Trade.BeginTrade (entityId);
				//debugs.text ="交易目标ID是 :" + entityId;
	...
	public void Open_TradeWindows(Int32 TargetID)
	{
				debugs.text = "我以收到服务器的响应";
				Trade.activate ();
				Trade.SetTitel (TargetID); 
